...
require_once(dirname(__FILE__)."/db/AbsDBResult.php");
require_once(dirname(__FILE__)."/db/ConnMgr.php");
require_once(dirname(__FILE__)."/db/DBMan.php");
require_once(dirname(__FILE__)."/db/DBResult.php");
require_once(dirname(__FILE__)."/db/DBStmt.php");
require_once(dirname(__FILE__)."/db/IBalancer.php");
require_once(dirname(__FILE__)."/db/IHostSelector.php");
require_once(dirname(__FILE__)."/db/ISQL.php");
require_once(dirname(__FILE__)."/db/IStatusMan.php");
require_once(dirname(__FILE__)."/db/RALLog.php");
require_once(dirname(__FILE__)."/db/RandBalancer.php");
require_once(dirname(__FILE__)."/db/RandSelector.php");
require_once(dirname(__FILE__)."/db/SplitDB.php");
require_once(dirname(__FILE__)."/db/SplitDBResult.php");
require_once(dirname(__FILE__)."/db/SQLAssember.php");
require_once(dirname(__FILE__)."/db/SQLTemplate.php");
require_once(dirname(__FILE__)."/db/StatusManFile.php");
require_once(dirname(__FILE__)."/db/StmtResult.php");
...
class Bd_DB
{
	...
	*/
    public function connect($host, $uname = NULL, $passwd = NULL, $dbname = NULL, $port = NULL, $flags = 0)
    {
        ...
        if(!$this->isConnected){
        	Bd_Db_RALLog::warning(RAL_LOG_SUM_FAIL, "Bd_DB", $dbname, "connect", "$host:$port", 0, 0, 0, 0, 0, $dbname, "", $this->mysql->errno, $this->mysql->error);
        }
    ...
    public function ralConnect($strName)
    {
    	...
    	if(!$this->isConnected){
    		Bd_Db_RALLog::warning(RAL_LOG_SUM_FAIL, "Bd_DB", $this->$dbConf['dbname'], "connect", "{$this->$dbConf['host']}:{$this->$dbConf['port']}", 
    			0, 0, 0, 0, 0, $this->$dbConf['dbname'], "", $this->mysql->errno, $this->mysql->error);
    	}
	...
	*/
    public function reconnect()
    {
        ...
        if(!$this->isConnected){
        	Bd_Db_RALLog::warning(RAL_LOG_SUM_FAIL, "Bd_DB", $conf['dbname'], "reconnect", "{$conf['host']}:{$conf['port']}", 0, 0, 0, 0, 0, $conf['dbname'], "", $this->mysql->errno, $this->mysql->error);
        }
        ...
        */
        ...
        if(!is_string($sql))
        {
            ...
            if(!($sql instanceof Bd_Db_ISQL) || !($sql = $sql->getSQL()))
            {
				...
				Bd_Db_RALLog::warning(RAL_LOG_SUM_FAIL, "Bd_DB", $this->dbConf['dbname'], "query", "{$this->dbConf['host']}:{$this->dbConf['port']}", 
        ...
        {				
            ...
            if(!$ret)
            {
				...
				Bd_Db_RALLog::warning(RAL_LOG_SUM_FAIL, "Bd_DB", $this->dbConf['dbname'], "query", "{$this->dbConf['host']}:{$this->dbConf['port']}",
					0, 0, 0, 0, 0, $this->dbConf['dbname'], $logPara['sql'], $this->mysql->errno,  $this->mysql->error);
...
//                echo "unknow type: $type_char\n";
	...
	*/
    public function prepare($query, $getRaw = false)
    {
        ...
        if($stmt === false)
        {
        	Bd_Db_RALLog::warning(RAL_LOG_SUM_FAIL, "Bd_DB", $this->dbConf['dbname'], "prepare", "{$this->dbConf['host']}:{$this->dbConf['port']}",
...
/*
$t = new  Bd_DB();
$t->connect('10.81.37.40', 'root', 'root', 'mco_developer_openplatform', 4044);

$sql = "select * from app_base_stage limit 2";
