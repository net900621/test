...
class Bd_Db_ConnMgr
{
    ...
    private static function _init($clusterName)
    {
        ...
        if(self::$_conf  === false || $clusterName == '')
        {
            ...
            Bd_Db_RALLog::warning(RAL_LOG_SUM_FAIL, "ConnMgr", $clusterName, "init", "",
            	0, 0, 0, 0, 0, $clusterName, "", self::$_error['errno'], self::$_error['error']);
            return false;
        }
    ...
    */
    ...
    {	
        ...
        if($hookBeforeInit === false){
            ...
            //Bd_Log::warning(self::$_error['error'],self::$_error['errno']);
            Bd_Db_RALLog::warning(RAL_LOG_SUM_FAIL, "ConnMgr", $clusterName, "getConn", "",
            	0, 0, 0, 0, 0, $clusterName, "", self::$_error['errno'], self::$_error['error']);
            return false;
        }
        if($hookBeforeInit != NULL){
            ...
            if(is_callable($hookBeforeInit)){
                ...
                //Bd_Log::warning(self::$_error['error'],self::$_error['errno']);
                Bd_Db_RALLog::warning(RAL_LOG_WARN, "ConnMgr", $clusterName, "getConn", "",
                	0, 0, 0, 0, 0, $clusterName, "", self::$_error['errno'], self::$_error['error']);
            }
        }     
...
//                         );
            ...
            for($i = 1; $i <= $conf['retry_times']; $i++)
            {
                ...
                Bd_Db_RALLog::warning(RAL_LOG_DEBUG, "ConnMgr", $clusterName, "getConn", "",
                	0, 0, 0, 0, 0, $clusterName, "", 0, "retry times: $i");
                ...
                if($ret){
                    if(empty($conf['charset']) || $db->charset($conf['charset'])){
                        ...
                        Bd_Db_RALLog::warning(RAL_LOG_DEBUG, "ConnMgr", $clusterName, "getConn", "",
                        	0, 0, 0, 0, 0, $clusterName, "", 0, 'Set charset failed');
                    }
                }
            }
	        //connect failed
