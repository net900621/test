...
class BD_DB_SplitDB
{
	...
	public function __construct($strDBName,$strConfPath,$strConfFilename)
	{
		...
		$this->reset();
		$this->_timer = new Bd_Timer(false,Bd_Timer::PRECISION_US);
		$this->_connectTime = 0;
		$this->_queryTime = 0;
		$this->_transTime = 0;
	}
	...
	*/
    ...
    public function doSql($strSql,$fetchType = Bd_DB::FETCH_ASSOC,$bolUseResult = false)
    {
        ...
        if(NULL === $arrSql || $arrSql['err_no']!==0)
        {
        	...
        	Bd_Db_RALLog::warning(RAL_LOG_SUM_FAIL, "SplitDB", $this->_strDBName, 'transform', '', $this->_timer->getTotalTime(Bd_Timer::PRECISION_MS),
        ...
        {
        	Bd_Db_RALLog::warning(RAL_LOG_SUM_FAIL, "SplitDB", $this->_strDBName, 'check', '', $this->_timer->getTotalTime(Bd_Timer::PRECISION_MS),
        ...
        foreach($this->hkBeforeQ as $arrCallback)
        {
        	...
        	{
        		Bd_Db_RALLog::warning(RAL_LOG_SUM_FAIL, "SplitDB", $this->_strDBName, 'query', '', $this->_timer->getTotalTime(Bd_Timer::PRECISION_MS),
        ...
        foreach ($arrSql['sqls'] as $oneSql)
        {
        	...
        	if(is_bool($res) || $res === NULL)
        	{
        		...
        		if(!$ok)
        		{
        			...
        			else if ( ($arrSql['sql_type'] === BD_DB_SplitDB::SPLITDB_SQL_UPDATE || $arrSql['sql_type'] === BD_DB_SplitDB::SPLITDB_SQL_DELETE) &&
        					$this->_arrMysql[ $strDBInstanceName ]->errno === BD_DB_SplitDB::MYSQL_ERR_TABLE_NOT_EXIST &&
        					$arrSql['table_split_type'] === BD_DB_SplitDB::SPLITDB_SPLIT_TYPE_RANGE )
        			{
